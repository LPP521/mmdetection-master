...
def anchor_target_single(flat_anchors,
    ...
    if len(gt_bboxes) != 0:
        ...
        # bbox_targets bbox_weights labels 不用做处理 均为0
        # 当无gt时 均为背景框 只参与box_cls的训练
        # 生成随记idx 用于随记采样
        idx = torch.randperm(len(anchors))
        if sampling:
            neg_inds = idx[:cfg['sampler']['num']]  # 256
        else:
            neg_inds = idx  # abchor数目 focal loss
        label_weights[neg_inds] = 1.0
        # 用于统计 无实际作用
        pos_inds = torch.zeros((0))
